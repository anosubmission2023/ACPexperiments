...
def run_pred_experiment(dataset_name, model_name, random_seed, base_path, estimator='mean'):
    ...
    v_train = quantile_score(quantile_estimator.predict(x_train[idx_train]), y_train[idx_train])
    v_cal = quantile_score(quantile_estimator.predict(x_train[idx_cal]), y_train[idx_cal])
    v_test = quantile_score(quantile_estimator.predict(x_test), y_test)
    ...
    # # SPLIT conformal
    # cp_split = ConformalPred(model=quantile_estimator,
    #                          method='split',
    ...
    y_lower['LCP-RF'] = pred_mean[:, 0] - r['LCP-RF']
    y_upper['LCP-RF'] = pred_mean[:, 1] + r['LCP-RF']

    r['LCP-RF-Group'], y_lcp_group = acv_xplainer.predict_rf_lcp_bygroup_train(x_test, level)
    y_lower['LCP-RF-Group'] = pred_mean[:, 0] - r['LCP-RF-Group']
    y_upper['LCP-RF-Group'] = pred_mean[:, 1] + r['LCP-RF-Group']

    r['QRF_train'] = acv_xplainer.predict_qrf_r(x_test)
    y_lower['QRF_train'] = pred_mean[:, 0] - r['QRF_train']
    y_upper['QRF_train'] = pred_mean[:, 1] + r['QRF_train']
    ...
    y_lower['SPLIT-G'] = pred_mean[:, 0] - r_split_group
    y_upper['SPLIT-G'] = pred_mean[:, 1] + r_split_group
    ...
    if 'simulation' in dataset_name or 'gen' in dataset_name or "shift" in dataset_name:
        for method in methods:
            plot_func(x=x_test[:, 0].reshape(-1, 1),
                      ...
                      split_lower=y_lower['CQR'],
                      split_upper=y_upper['CQR'])
...
run_pred_experiment('bike', 'linear', random_seed=2022, base_path='/home/xxxxx/saved_acp_q/', estimator='quantile')
