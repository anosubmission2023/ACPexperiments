...
@cython.nonecheck(False)
@cython.cdivision(True)
cpdef single_compute_quantile_rf(double[:] & x, double & y_x,  double[:, :] & data, double[::1] & y_data, vector[int] & S,
        int[:, :] & features, double[:, :] & thresholds,  int[:, :] & children_left, int[:, :] & children_right,
        int[::1] & max_depth, int & min_node_size, int & classifier, double & t, double[::1] & weights, double[::1] & samples, double[::1] & samples_child, double & quantile, int & min_node_size_intersection):
    ...
    cdef unsigned int b, level, it_node, i, prev_trees, s_prev
    cdef vector[int] nodes_level, nodes_child

    cdef samples_intersection
    samples_intersection = np.ones((n_trees, data.shape[0]))
    ...
    for b in tqdm(range(n_trees)):
        ...
        for i in range(N):
            samples[i] = 1
            samples_child[i] = 1
        # TODO: use the true max_depth of each tree
        for level in range(max_depth[b]):
            print(b, level, list(nodes_level), s)
            if not nodes_level.size() > 0:
                break
            for it_node in range(nodes_level.size()):

                if features[b, nodes_level[it_node]] < 0:
                    break
                s = 0
                if std_find[vector[int].iterator, int](S.begin(), S.end(), features[b, nodes_level[it_node]]) != S.end():
                    if x[features[b, nodes_level[it_node]]] <= thresholds[b, nodes_level[it_node]]:
                ...
                # stop = False
                # s_prev = 0
                # for prev_trees in range(0, b):
                #    s_prev = 0
                #    for i in range(N):
                #        s_prev += samples_intersection[b, i] * samples_child[i]
                #    if s_prev < min_node_size_intersection:
                #        stop = True
                #        break
                ...
                if b != 0:
                    for i in range(N):
                        samples_intersection[b, i] = samples_child[i]

                    s_prev = np.sum(np.prod(samples_intersection[:b+1], axis=0))
                    if s_prev < min_node_size_intersection:
                        stop = True
        ...
        for i in range(N):
            s += samples[i]
            samples_intersection[b, i] = samples[i]
...
cpdef compute_quantile_rf(double[:, :] X, double[::1] y_X,  double[:, :] data, double[::1] y_data, vector[vector[int]] S,
        int[:, :] features, double[:, :] thresholds,  int[:, :] children_left, int[:, :] children_right,
        int[::1] max_depth, int min_node_size, int & classifier, double & t, double & quantile, int & min_node_size_intersection):

        cdef int N = X.shape[0]
        cdef double[::1] sdp = np.zeros(N)
        cdef samples, samples_child
        weights = np.zeros((N, data.shape[0]))
        samples = np.ones((N, data.shape[0]))
        samples_child = np.ones((N, data.shape[0]))
        cdef int i
        for i in range(N):
            sdp[i], weights[i] = single_compute_quantile_rf(X[i], y_X[i], data, y_data, S[i],
                        features, thresholds, children_left, children_right,
                        max_depth, min_node_size, classifier, t, weights[i, :], samples[i, :], samples_child[i, :], quantile, min_node_size_intersection)
        return np.array(sdp), weights
...
@cython.nonecheck(False)
@cython.cdivision(True)
cpdef compute_sdp_weight_verbose(double[:, :] X, double[::1] y_X,  double[:, :] data, double[::1] y_data, vector[vector[int]] S,
        int[:, :] features, double[:, :] thresholds,  int[:, :] children_left, int[:, :] children_right,
        int max_depth, int min_node_size, int & classifier, double[:, :] & t, double & pi):
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef double single_compute_weights(double[:] & x, double & y_x,  double[:, :] & data, double[::1] & y_data,
        int[:, :] & features, double[:, :] & thresholds,  int[:, :] & children_left, int[:, :] & children_right,
        int & max_depth, int & min_node_size, int & classifier, double[:] & t, double[:, :] & partition,
        double[:] & w) nogil:

    # TODO: optimize the code to go through the tree by recursion
    cdef unsigned int n_trees = features.shape[0]
    cdef unsigned int N = data.shape[0]
    cdef double s, sdp
    cdef int o
...
@cython.nonecheck(False)
@cython.cdivision(True)
cpdef compute_weights_verbose(double[:, :] X, double[::1] y_X,  double[:, :] data, double[::1] y_data,
        int[:, :] features, double[:, :] thresholds,  int[:, :] children_left, int[:, :] children_right,
        int max_depth, int min_node_size, int & classifier, double[:, :] & t, double & pi):
        ...
        for i in tqdm(range(N)):
            sdp[i] = single_compute_weights(X[i], y_X[i], data, y_data,
                        features, thresholds, children_left, children_right,
                        max_depth, min_node_size, classifier, t[i], partition_byobs[i], weights[i])
