...
def return_possible_values(rule, rule_data, sdp_all, pi_level):
    """
    return the possible values of the hyper-rectangles using the neighboring leaves (rule_data)
    Args:
        rule (numpy.ndarray): 2-D, the target rule
        rule_data (numpy.ndarray): 3-D, the neighboring leaves
        sdp_all (numpy.ndarray): 1-D, the SDP(y_rule) of the neighboring leaves
        pi_level (double): the minimum probability or (SDP) of the neighboring leaves

    Returns:
        left_by_var (list): the possible values for the lower bound of the hyperrectangle
        right_by_bar (list): the possible values for the upper bound of the hyperrectangle
    """
    d = rule.shape[0]
...
def generate_newrule(rule, left_by_var, right_by_var, p_best=0.6):
    """
    Generate new rule using the possible values (left_by_var, right_by_var)
    Args:
        rule (numpy.ndarray): 2-D, the target rule
        left_by_var (list): the possible values for the lower bound of the hyperrectangle
        right_by_bar (list): the possible values for the upper bound of the hyperrectangle
        p_best (double): with probability=p_best, sample randomly, otherwise try to find the maximal rule in all
                        the directions

    Returns:
        new_rule (numpy.ndarray): 2-D, the new rule
    """
    d = rule.shape[0]
    rule_c = rule.copy()
...
def check_rule(rule, x_train, bad_sample):
    """
    check if the rule is valid (i.e. if the generated rule is include in the union of all compatible rules)
    Args:
        rule (numpy.ndarray): 2-D, the target rule
        x_train (numpy.ndarray): 2-D, training sample
        bad_sample (bool): Training observations that are not in the union of the compatibles rules

    Returns:
        valid (bool): True, if the rule is in the union of the compatible rules
    """
...
def generate_valid_rule(rule, left_by_var, right_by_var, x_train, bad_sample, p_best=0.9, n_try=100):
    """
    Try to compute the union of the compatible rules by extending the target rule.
    Args:
        rule (numpy.ndarray): 2-D, the target rule
        left_by_var (list): the possible values for the lower bound of the hyperrectangle
        right_by_bar (list): the possible values for the upper bound of the hyperrectangle
        ...
        p_best (double): with probability=p_best, sample randomly, otherwise try to find the maximal rule in all
                        the directions
        n_try (int): maximal number of tries to generate a valid rule
...
def generate_rules(rule, left_by_var, right_by_var, x_train, bad_sample, p_best=0.6, n_try=100, n_gen=10):
    """
    Generate n_gen valid rules using the function (generate_valid_rule)
    Args:
        rule (numpy.ndarray): 2-D, the target rule
        left_by_var (list): the possible values for the lower bound of the hyperrectangle
        right_by_bar (list): the possible values for the upper bound of the hyperrectangle
        ...
        p_best (double): with probability=p_best, sample randomly, otherwise try to find the maximal rule in all
                        the directions
        n_try (int): maximal number of tries to generate a valid rule
        n_gen (int): number of generated valid rules

    Returns:
        return n_gen valid rules
    """
...
def volume_rectangle(rec, max_values, min_values):
    """
    Compute the volume of the hyperrectangle while limiting the maximum and minimum values
    """
    d = rec.shape[0]
    rec_c = rec.copy()
...
def rules_simulated_annealing(value_function, rule, left_by_var, right_by_var, x_train, bad_sample, p_best=0.9,
    ...
    """
    Compute the maximal hyper-rectangle (using value_function as criteria). It extends the target rule using the
    possible values of the neighboring rules (left_by_var, and right_by_var).
    Args:
        value_function (function): it takes 3 arguments (rectangle, max_values, min_values) and return a score
        rule (numpy.ndarray): 2-D, the target rule
        left_by_var (list): the possible values for the lower bound of the hyperrectangle
        right_by_bar (list): the possible values for the upper bound of the hyperrectangle
        x_train (numpy.ndarray): 2-D, training sample
        ...
        p_best (double): with probability=p_best, sample randomly, otherwise try to find the maximal rule in all
                        the directions
        n_try (int): maximal number of tries to generate a valid rule
        batch (int): number of generated valid rules
        max_iter (int): maximal number of iteration of the temperature of the simulated annealing algorithm
        temp (int): the temperature of the simulated annealing algorithm
        max_iter_convergence (int): maximal number of iteration without improvement

    Returns:
        best_rule (numpy.ndarray): the best (maximal) rule in the union
        best_eval (double): the score given by the value_function of the best rule
    """
    max_values = [np.max(x_train[:, i]) for i in range(x_train.shape[1])]
    min_values = [np.min(x_train[:, i]) for i in range(x_train.shape[1])]

    best = generate_rules(rule, left_by_var, right_by_var, x_train, bad_sample, p_best=1, n_try=n_try, n_gen=1)
...
def rules_by_annealing(value_function, rules, rules_data, sdp_all, x_train, pi_level=0.9, p_best=0.6, n_try=50,
    ...
    """
    Compute the maximal hyper-rectangle (using value_function as criteria) for each observations. It extends the target
    rule using the possible values of the neighboring rules (left_by_var, and right_by_var).
    Args:
        value_function (function): it takes 3 arguments (rectangle, max_values, min_values) and return a score
        rule (numpy.ndarray): 2-D, the target rule
        left_by_var (list): the possible values for the lower bound of the hyperrectangle
    ...
    Returns:
        sr (numpy.ndarray): the best (maximal) rule in the union
        sr_eval (double): the score given by the value_function of the best rule
    """
    sr, sr_eval = [], []
...
#
#
# def sorted_w(w_cali, r_cali):
#     w_cali_sorted = np.zeros(shape=w_cali.shape)
#     argsort_ycali = np.argsort(r_cali)
#
#     for i in range(w_cali.shape[0]):
#         w_cali_sorted[i] = w_cali[argsort_ycali[i]]
#     return w_cali_sorted, argsort_ycali
#
#
# def return_k(S, alpha):
#     for k in range(S.shape[0]):
#         if S[k] >= alpha:
#             return k - 1
#
#
# def return_w_cali(x, y, x_cali, r_cali, w_cali):
#     w_o = np.zeros(shape=(x_cali.shape[0] + 1, x_cali.shape[0] + 1))
#     w_o[:-1, :-1] = w_cali
#     x_cali_new = np.concatenate([x_cali, x.reshape(1, -1)])
#     r_cali_new = np.concatenate([r_cali.reshape(-1, 1), y.reshape(1, -1)]).reshape(-1)
#     w_corrected = acv_xplainer.compute_forest_weights_cali(x, y, x_cali_new, r_cali_new, w_o)
#     return w_corrected, x_cali_new, r_cali_new
#
#
# def compute_partition(w_cali, r_cali):
#     w_cali_sorted, argsort_ycali = sorted_w(w_cali, r_cali)
#
#     w_cali_aug = np.zeros(shape=(w_cali_sorted.shape[0] + 1, w_cali_sorted.shape[1]))
#     w_cali_aug[1:, :] = w_cali_sorted
#     argsort_ycali_aug = [i + 1 for i in argsort_ycali]
#
#     theta = np.cumsum(w_cali_aug, axis=1)
#     a_1, p_1 = [], []
#     a_2, p_2 = [], []
#     a_3, p_3 = [], []
#     for i in range(r_cali.shape[0]):
#
#         if theta[i, l(i)] + w_cali[i, -1] < theta[-1, l(i)]:
#             a_1.append(i)
#             p_1.append(theta[i, l(i)] + w_cali[i, -1])
#
...
#             p_3.append(l(i))
#
#     p_1_sort = np.sort(p_1)
#     p_2_sort = np.sort(p_2)
#     p_3_sort = np.sort(p_3)
#
#     c_1, c_2, c_3 = 0, 0, 0
#     L_1, L_2, L_3 = len(a_1), len(a_2), len(a_3)
#
#     S = np.zeros(shape=r_cali.shape[0])
#     for k in range(r_cali.shape[0]):
...
#             c_3 = c_3 + 1
#         S[k] = (c_1 + c_2 + c_3) / (r_cali.shape[0])
#     return S
#
...
# def train_conditional_calibration(x_test, y_test, pred, x_cali, r_cali,
#                                   r_lcp, s_lcp, quantile):
#
#     y_lower = pred - r_lcp
#     y_upper = pred + r_lcp
#     coverage = compute_coverage(y_test, y_lower, y_upper)
#
#     r_lcp_train = np.zeros(shape=r_lcp.shape)
#     k = 0
#
#     while coverage < quantile:
#         k += 1
#         for i in range(x_test.shape[0]):
#             k_star = s_lcp[i] + k
#             if k_star <= r_cali.shape[0] and k_star > 0:
#                 r_lcp_train[i] = np.sort(r_cali)[k_star - 1]
#             elif k_star == 0:
#                 r_lcp_train[i] = np.min(r_cali)
#             else:
#                 r_lcp_train[i] = np.max(r_cali)
#
#         y_lower = pred - r_lcp_train
#         y_upper = pred + r_lcp_train
#         coverage = compute_coverage(y_test, y_lower, y_upper)
#     return k, coverage

# w_cali = acv_xplainer.compute_forest_weights(x_cali, r_cali, x_cali, r_cali) # data_bground is the background dataset that is used for the estimation. It should be the training samples.
#
# for i in range(50):
#     w_corrected, x_cali_new, r_cali_new = return_w_cali(x_test[i], y_test[i], x_cali, r_cali, w_cali)
#     S = compute_partition(w_corrected, r_cali_new)
#     print(return_k(S, 0.6))
