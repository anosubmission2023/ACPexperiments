...
class ACXplainer:
    ...
    def fit_calibration(self, x_cali, y_cali, model, bygroup=False, training_conditional=False, qrf_calibration=True, training_conditional_one=False,
                        min_scale=-1, constructor='continuous_normalized', max_scale=0.5, n_scale=100, n_tries=50,
                        method='leiden', quantile=0.9, score_type='mean', only_qrf=True, n_steps_qrf=40, n_iter_qrf=50):

        self.x_cali, self.y_cali = check_X_y(x_cali, y_cali, dtype=np.double)
        self.model_cali = model
        self.score_type = score_type
        self.pred_cali = self.model_cali.predict(self.x_cali).astype(np.double)
        self.only_qrf = only_qrf

        if self.score_type == 'mean':
            self.r_cali = mean_score(self.pred_cali, self.y_cali).astype(np.double)
        elif self.score_type == 'quantile':
            self.r_cali = quantile_score(self.pred_cali, self.y_cali).astype(np.double)
        else:
            raise ValueError('Nonconformity score available is mean or quantile')
        self.r_cali = as_float_array(self.r_cali).astype(np.double)

        if qrf_calibration:
            print('Training calibration of QRF')
            self.alpha_star, self.coverage_qrf, self.idx_marg, self.idx_train = self.fit_qrf_calibration(self.x_cali, self.r_cali, self.y_cali,
                                                                          self.pred_cali,
                                                                          quantile, steps=n_steps_qrf, n_iter=n_iter_qrf)
            if self.only_qrf:
                return
        ...
        self.w_cali = self.compute_forest_weights(self.x_cali, self.r_cali, self.x_cali,
                                                  self.r_cali)
        self.check_is_calibrate = True

        if training_conditional:
            print('Training-conditional calibration of RF-LCP')
            ...
            self.k_cali, self.coverage_cali = self.train_conditional_calibration(self.x_cali_train, self.r_cali_train,
                                                                                 self.pred_cali_train, self.x_cali,
                                                                                 self.r_cali,
                                                                                 self.r_lcp_train_cali,
                                                                                 self.s_lcp_train_cali, quantile)
            self.check_is_training_conditional = True

        if training_conditional_one:
            print('Training-conditional-one calibration of RF-LCP')

            self.k_cali_one, self.coverage_cali_one = self.train_conditional_calibration_one(self.x_cali_train, self.r_cali_train,
                                                                                 self.pred_cali_train, self.x_cali,
                                                                                 self.r_cali,
                                                                                 self.r_lcp_train_cali,
                                                                                 self.s_lcp_train_cali, quantile)
        ...
        if bygroup:
            ...
            if training_conditional:
                ...
                self.k_cali_bygroup, self.coverage_cali_bygroup = \
                    self.train_conditional_calibration_bygroup(self.x_cali_train, self.r_cali_train,
                                                               self.pred_cali_train,
                                                               self.r_lcp_bygroup, self.s_lcp_bygroup,
                                                               quantile)
                self.check_is_training_conditional_bygroup = True

    def fit_qrf_calibration(self, x_calibration, r_calibration, y_calibration, pred_calibration,
                            quantile, steps=40, n_iter=20):
        best_idx_marg = []
        best_idx_train = []
        best_alpha = []
        best_coverage = []
        for z in tqdm(range(n_iter)):
            idx = np.random.permutation(x_calibration.shape[0])
            n_half = int(np.floor(x_calibration.shape[0] * 0.5))
            x_cali = x_calibration[idx[:n_half]]
            r_cali = r_calibration[idx[:n_half]]
            y_cali = y_calibration[idx[:n_half]]

            x_cali_train = x_calibration[idx[n_half:]]
            r_cali_train = r_calibration[idx[n_half:]]
            y_cali_train = y_calibration[idx[n_half:]]
            pred_cali_train = pred_calibration[idx[n_half:]]

            self.qrf.fit(x_cali, r_cali)

            alpha_sets = np.linspace(quantile, 1, steps)
        ...
        x_cali = x_calibration[best_idx_marg[best_id]]
        r_cali = r_calibration[best_idx_marg[best_id]]
        self.qrf.fit(x_cali, r_cali)
    ...
    def predict_rf_lcp(self, x_test, quantile):
        ...
        return cyext_acv.compute_rf_lcp(x_test, self.x_cali, self.r_cali,
                                        self.w_cali, quantile, self)

    def predict_rf_lcp_support(self, x_test, quantile):
        if not self.check_is_calibrate:
            raise ValueError('You need to fit the calibration before')
        return cyext_acv.compute_rf_lcp_support(x_test, self.x_cali, self.r_cali,
                                                self.w_cali, quantile, self)
    ...
    def train_conditional_calibration(self, x_test, y_test, pred, x_cali, r_cali,
                                      r_lcp, s_lcp, quantile):

        # self.w_cali_train = np.zeros(shape=(x_test.shape[0], x_cali.shape[0]+1))
        # w_cali_train_sorted = np.zeros(shape=self.w_cali_train.shape)
        #
        # for i in tqdm(range(x_test.shape[0])):
        #     w_corrected, x_cali_new, r_cali_new = cyext_acv.return_w_cali_py(x_test[i], x_cali, r_cali, self.w_cali, self)
        #     argsort_y = np.argsort(r_cali_new)
        #     for j in range(r_cali_new.shape[0]):
        #         self.w_cali_train[i, j] = w_corrected[-1, j]
        #         w_cali_train_sorted[i, j] = w_corrected[-1, argsort_y[j]]
        #
        # w_argsort = []
        # start = []
        # for i in range(w_cali_train_sorted.shape[0]):
        #     w_argsort.append(np.nonzero(w_cali_train_sorted[i])[0])
        #     start.append(find_index(w_argsort[-1], s_lcp[i]))
        if self.score_type == 'mean':
    ...
    def train_conditional_calibration_one(self, x_test, y_test, pred, x_cali, r_cali,
                                          r_lcp, s_lcp, quantile):
    ...
    def train_conditional_calibration_bygroup(self, x_test, y_test, pred,
                                              r_lcp, s_lcp, quantile):
        ...
        if self.score_type == 'mean':
            ...
            y_lower = pred[:, 0] - r_lcp
            y_upper = pred[:, 1] - r_lcp
        ...
        while coverage < quantile:
            ...
            if self.score_type == 'mean':
                ...
                y_lower = pred[:, 0] - r_lcp_train
                y_upper = pred[:, 1] + r_lcp_train
            coverage = compute_coverage(self.y_cali_train, y_lower, y_upper)
        return k, coverage
    ...
    def compute_weights(self, X, y, data, y_data, min_node_size=5, t=None, pi_level=0.95,
                        verbose=False, algo2=False, p_best=0.85, n_try=50,
                        batch=50, max_iter=200, temp=1, max_iter_convergence=10):
        """
         Estimate the maximal rule-based explanations of a set of samples X given Sufficient Explanations S using the consistent estimator
        ...
        sdp, rules, sdp_all, rules_data, w = cyext_acv.compute_weights_verbose(X, y, data, y_data,
                                                                               self.ACXplainer.features,
                                                                               self.ACXplainer.thresholds,
                                                                               self.ACXplainer.children_left,
                                                                               self.ACXplainer.children_right,
    ...
    def compute_forest_weights(self, X, y, data, y_data, min_node_size=5, t=None, pi_level=0.95,
                               verbose=False, algo2=False, p_best=0.85, n_try=50,
                               batch=50, max_iter=200, temp=1, max_iter_convergence=10):
        """
         Estimate the maximal rule-based explanations of a set of samples X given Sufficient Explanations S using the consistent estimator
    ...
    def compute_forest_weights_cali(self, X, data, y_data, weights, min_node_size=5, t=None, pi_level=0.95,
                                    verbose=False, algo2=False, p_best=0.85, n_try=50,
                                    batch=50, max_iter=200, temp=1, max_iter_convergence=10):
        """
         Estimate the maximal rule-based explanations of a set of samples X given Sufficient Explanations S using the consistent estimator
        ...
        # X, y = check_X_y(X, y, dtype=np.double)
        # data, y_data = check_X_y(data, y_data, dtype=np.double)
        # y_data = as_float_array(y_data).astype(np.double)

        # self.check_is_explainer()
        # for i in range(X.shape[0]):
        #     data[-1, :] = X[i]
        #     y_data[-1] = y[i]
        #     w = cyext_acv.compute_forest_weights_cali_verbose(X[i], y[i], data, y_data,
        #                                                                self.ACXplainer.features,
        #                                                                self.ACXplainer.thresholds,
        #                                                                self.ACXplainer.children_left,
        #                                                                self.ACXplainer.children_right,
        #                                                                min_node_size, weights)
    ...
    def sufficient_cxpl_rf(self, X, y, data, y_data, min_node_size=5, t=None,
        ...
        if not self.classifier and t is None:
            ...
            t = np.zeros(shape=(X.shape[0], 2))
        c_expl, cdp_expl, cdp_global = cyext_acv.sufficient_cxpl_rf(X, y, data, y_data, self.ACXplainer.features,
                                                                    self.ACXplainer.thresholds,
                                                                    self.ACXplainer.children_left,
                                                                    self.ACXplainer.children_right,
                                                                    self.ACXplainer.max_depth, min_node_size,
    ...
    def compute_quantile_rf(self, X, y, data, y_data, S, min_node_size=5, classifier=1, t=20, quantile=95,
        ...
        y_quantiles, y_weights = cyext_acv.compute_quantile_rf(X, y, data, y_data, S, self.ACXplainer.features,
                                                               self.ACXplainer.thresholds,
                                                               self.ACXplainer.children_left,
                                                               self.ACXplainer.children_right, self.depth,
                                                               min_node_size, classifier, t,
                                                               quantile, min_node_size_intersection)
        return y_quantiles, y_weights
